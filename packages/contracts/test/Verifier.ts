...
describe('VerifyProof', function () {
  ...
  this.beforeAll(async () => {
    ...
    return ({
      ...
      c,
      Input: publicInputs,
    } = await exportCallDataGroth16FromPCD(pcd))
  })
  ...
  async function deployOneYearLockFixture() {
    ...
    const anonAadhaar = await AnonAadhaarContract.deploy(
      _verifierAddress,
      pubkeyHashBigInt,
    )

    await anonAadhaar.waitForDeployment()
    const _AnonAadhaarAddress = anonAadhaar.getAddress()
  ...
  describe('AnonAadhaar Contract', function () {
    describe('Verifier', function () {
      it('Should return true for a valid PCD proof', async function () {
        const { anonAadhaar } = await loadFixture(deployOneYearLockFixture)

        expect(
          await anonAadhaar.verifyProof(a, b, c, publicInputs, 1),
        ).to.be.equal(true)
      })
      ...
      it('Should revert for a wrong signal', async function () {
        ...
        await expect(anonAadhaar.verifyProof(a, b, c, publicInputs, 40)).to
          .reverted
      })
    })
  })
  ...
  describe('AnonAadhaar Vote', function () {
    describe('Vote for a proposal', function () {
      ...
      it('Should verify a proof with right address in signal', async function () {
        ...
        const args = await generateArgs(testQRData, certificate, user.address)
        ...
        await expect(
          anonAadhaarVote.voteForProposal(1, a, b, c, Input, user.address),
